...
const ConnectingWires: React.FC<Probs> = ({
  plugs,
  wires,
  stompClient,
  lobbyId,
    ...
    if (ctx) {
      ...
      plugs.forEach((plug) => {
        if (wires[plug[0]]) {
          ...
          ctx.lineTo(
            canvas.width - offset,
            start_y + plug[0] * button_distance
          );
  ...
  return (
    <div className="flex h-72 w-96">
      <div
        id="leftButtons"
        className="flex flex-col justify-between absolute left-7"
      >
        {plugs.map((plug, index) => (
          <ConnectingWiresButton
            onClick={() => {
              connectingWiresButtonClick(plug[0], "left");
            }}
            wire={colour[plug[0]]}
      ...
      ></canvas>
      {taskCompleted && (
        <h3 className="absolute z-50 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center font-extrabold text-3xl -rotate-12 py-2 bg-red-300 shadow-lg shadow-red-500">
          Task Completed
        </h3>
      )}
      <div
        id="rightButtons"
        className="flex flex-col justify-between absolute right-7"
      >
        {plugs.map((plug, index) => (
          <ConnectingWiresButton
            onClick={() => {
              connectingWiresButtonClick(plug[1], "right");
            }}
            wire={colour[plug[1]]}
            disabled={taskCompleted}
            key={index}
