...
const InGameButton: React.FC<ButtonProps> = ({ onClick, label, active }) => {
  return (
    <button 
      className={((active) ? "opacity-60 hover:bg-gray-700" : "opacity-30") + " bg-gray-500 text-white font-bold rounded-full w-20 h-20 absolute bottom-5 right-5 transform -translate-x-1/2 -translate-y-1/2"}
      onClick={onClick}
      disabled={!active}
    >
      {label}
